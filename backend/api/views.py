    ...
    for attempt in range(2): # Up to 2 passes if initial attempt hit rate limits
        for model in models:
            try:
                ...
                last_err = e
                # Skip immediately for auth errors ΓÇö retrying won't help
                if 'AuthenticationError' in err_str or 'invalid_api_key' in err_str.lower() or '401' in err_str:
                    print(f"[AI] Auth error for {model} ΓÇö check your API key in .env", flush=True)
                    continue
...
def _ensure_extracted_text(doc):
    ...
    try:
        ...
        if text.strip():
            # Text-based PDF ΓÇö save and return
            doc.extracted_text = text
            doc.save()
            return True

        # Scanned PDF ΓÇö use Gemini Vision OCR
...
class QuizView(APIView):
    ...
    def post(self, request, doc_id):
        try:
            ...
            # ΓöÇΓöÇ CACHE-FIRST: return instantly if we have pre-generated questions ΓöÇΓöÇ
            if quiz_type == "objective" and not force_regenerate:
                try:
                    ...
                    pass  # Cache miss ΓåÆ fall through to live generation
...
class StudentProfileView(APIView):
    ...
    def post(self, request):
        try:
            if hasattr(request.user, 'student_profile'):
                # User already has a profile ΓÇö update it
        ...
        for p in paragraphs:
            ...
            if text:
                if text.startswith('# '):
                    ...
                    # Clean markdown list
                    story.append(Paragraph(f"ΓÇó {text[2:]}", body_style))
                else:
                    text = text.replace('```', '')
...
class QuizPerformanceAnalysisView(APIView):
    """Generate an AI coaching report based on the user's quiz performance."""
    permission_classes = (IsAuthenticated,)
    ...
    def post(self, request):
        try:
            ...
            user_answers = request.data.get("user_answers", {})
            score = request.data.get("score", 0)
            total = request.data.get("total", len(quiz_data))

            if not quiz_data:
                return Response({"error": "Quiz data is required."}, status=400)

            # Build a summary of correct/incorrect questions with topics
            correct_topics = []
            incorrect_topics = []
            for i, q in enumerate(quiz_data):
                topic = q.get("topic", "General")
                ...
                if user_ans is not None and int(user_ans) == int(correct_ans):
            ...
            prompt = (
                f"You are an expert academic coach analyzing a student's quiz performance.\n\n"
                f"Quiz Results:\n"
                f"- Score: {score}/{total} ({percentage}%)\n"
                f"- Topics answered correctly: {', '.join(correct_topics) or 'None'}\n"
                f"- Topics answered incorrectly: {', '.join(incorrect_topics) or 'None'}\n\n"
                f"Analyze this performance and provide a brief, personalized coaching report. "
                f"You MUST respond with ONLY a valid JSON object (no markdown) with exactly these keys:\n"
                f"- 'strengths': A 2-3 sentence paragraph about what the student did well.\n"
                f"- 'weaknesses': A 2-3 sentence paragraph about the areas that need improvement.\n"
                f"- 'study_plan': A concise, actionable 3-5 step study plan to improve weak areas.\n\n"
                f"Response must be pure JSON only."
            )
                ...
                try:
                    ...
                    if start != -1 and end != -1:
                        try:
                            ...
                            pass
                    if attempt == 2:
                        # Return a graceful fallback report
                        return Response({
                            "strengths": f"You scored {percentage}% ({score}/{total}), showing solid understanding of {', '.join(correct_topics[:3]) or 'several topics'}.",
                            "weaknesses": f"Review the following topics which need more attention: {', '.join(incorrect_topics[:5]) or 'None identified'}.",
                            "study_plan": "1. Review the incorrect answers and their explanations.\n2. Re-read relevant sections of your notes.\n3. Attempt the quiz again after 24 hours."
                        })
                    continue
                except Exception as rate_err:
                    if ('429' in str(rate_err) or 'quota' in str(rate_err).lower()) and attempt < 2:
